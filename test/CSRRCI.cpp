...
DEFINE_TESTCASE(CSRRCI) {
    ...
    ASSERT(rd.u64 == expected, "Wrong CSRSCI result. Expected {:x} in RD {:x}, got {:x}", expected, sel_rd, rd.u64);

    expected &= ~sel_rs;
    ASSERT(csrs_updated[sel_csr] == expected, "Wrong CSRSCI result. Expected {:x} in CSR {:x}, got {:x}", expected, sel_csr, csrs_updated[sel_csr]);
