...
DEFINE_TESTCASE(BLTU, "BLTU") {
    ...
    ASSERT(vm.GetPC() == (base + 4), "BLTU {} == {} did jump. Expected {:x}, got {:x}", sel_rs1, sel_rs2, (base + 4), vm.GetPC());
    ...
    rs1.u64 = Random<Long>(0, LONG_MAX / 2);
    rs2.u64 = Random<Long>(LONG_MAX / 2, LONG_MAX);

    STEP_VMS(1);

    ASSERT(vm.GetPC() == (base + jump), "BLTU {} < {} did not jump. Expected {:x}, got {:x}", rs1.s64, rs2.s64, (base + jump), vm.GetPC());
    ...
    ASSERT(vm.GetPC() == (base + 4), "BLTU {} > {} did jump. Expected {:x}, got {:x}", rs2.s64, rs1.s64, (base + 4), vm.GetPC());
