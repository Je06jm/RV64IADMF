...
DEFINE_TESTCASE(CSRRWi) {
    ...
    ASSERT(rd.u64 == expected, "Wrong CSRSWI result. Expected {:x} in RD {:x}, got {:x}", expected, sel_rd, rd.u64);

    expected = (csrs_original[sel_csr] & ~0x1f) | sel_rs;
    ASSERT(csrs_updated[sel_csr] == expected, "Wrong CSRSWI result. Expected {:x} in CSR {:x}, got {:x}", expected, sel_csr, csrs_updated[sel_csr]);
