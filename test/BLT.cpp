...
DEFINE_TESTCASE(BLT, "BLT") {
    ...
    ASSERT(vm.GetPC() == (base + 4), "BLT {} == {} did jump. Expected {:x}, got {:x}", sel_rs1, sel_rs2, (base + 4), vm.GetPC());
    ...
    rs1.s64 = Random<SLong>(-10000000, -1);
    rs2.s64 = Random<SLong>(1, 10000000);

    STEP_VMS(1);

    ASSERT(vm.GetPC() == (base + jump), "BNE {} < {} did not jump. Expected {:x}, got {:x}", rs1.s64, rs2.s64, (base + jump), vm.GetPC());
    ...
    ASSERT(vm.GetPC() == (base + 4), "BNE {} > {} did jump. Expected {:x}, got {:x}", rs2.s64, rs1.s64, (base + 4), vm.GetPC());
